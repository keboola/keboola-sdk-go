...
The client is generated from the OpenAPI spec at `api/openapi.yaml` using [OpenAPI Generator](https://openapi-generator.tech).
...
Run from the `pkg/keboola/management/` directory:
...
java -jar /path/to/openapi-generator-cli.jar generate \
  ...
  --git-user-id=keboola \
  --git-repo-id=keboola-sdk-go/v2/pkg/keboola/management
```

> The `-o .` flag outputs directly into the current directory, overwriting the existing generated files in place.
>
> `--git-user-id` and `--git-repo-id` together set the Go module import path to
> `github.com/keboola/keboola-sdk-go/v2/pkg/keboola/management`, which replaces the
> default `github.com/GIT_USER_ID/GIT_REPO_ID` placeholder in generated imports.

### 3. Manual fixes after generation

After regenerating, apply the following manual fixes:
- **Token ID type**: Change `ID` field from `float32` to `string` in the token model
